...
class MySubscribeCallback(SubscribeCallback):
    ...
    def message(self, pubnub, message):
        if(message.message == "Currently_Occupied"):
        	print("Parking Slot Status: OCCUPIED by Vehicle-4")
        	reactor.stop()
        else:
...
parking_lat = 53.37708785680931 
parking_long = -6.25641477876585
...
def operate_signal():
	pubnub.publish().channel("parking-2").message(str(parking_values[0])).pn_async(my_publish_callback)
	time.sleep(10)
	#pubnub.publish().channel("parking-2").message(str(parking_values[1])).pn_async(my_publish_callback)
	#time.sleep(10)
