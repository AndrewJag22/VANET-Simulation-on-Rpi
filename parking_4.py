...
parking_lat = 53.379048
parking_long = -6.246855

parking_values = ["EMPTY"]

pubnub.add_listener(MySubscribeCallback())
pubnub.subscribe().channels("parking-4").execute()

parking_time = 5.0 #5 seconds
#Runs every 5 seconds
def operate_signal():
    pubnub.publish().channel("parking-4").message(str(parking_values[0])).pn_async(my_publish_callback)
    time.sleep(8)
    #pubnub.publish().channel("parking-4").message(str(parking_values[1])).pn_async(my_publish_callback)
    #time.sleep(15)
